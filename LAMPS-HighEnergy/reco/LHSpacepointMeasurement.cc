#include "LHSpacepointMeasurement.hh"
#include "TVector3.h"

using namespace std;

ClassImp(genfit::LHSpacepointMeasurement)

namespace genfit {
  //////////////////////////////////////////////////////////////////////////////////

  LHSpacepointMeasurement::LHSpacepointMeasurement()
  : SpacepointMeasurement()
  {}

  LHSpacepointMeasurement::LHSpacepointMeasurement(const KBHit* detHit, const TrackCandHit* hit)
  : SpacepointMeasurement()
  {
    TVector3 pos = detHit -> GetPosition();

    /// all genfit units in cm

    rawHitCoords_(0) = pos.X()/10.;
    rawHitCoords_(1) = pos.Y()/10.;
    rawHitCoords_(2) = pos.Z()/10.;

    //rawHitCov_(0,0) = 0.01;
    //rawHitCov_(1,1) = 0.01;
    //rawHitCov_(2,2) = 0.01;
    rawHitCov_(0,0) = 1;
    rawHitCov_(1,1) = 1;
    rawHitCov_(2,2) = 1;
    rawHitCov_(0,1) = 0;
    rawHitCov_(1,2) = 0;
    rawHitCov_(2,0) = 0;

    detId_ = hit->getDetId();
    hitId_ = hit->getHitId();

    this -> initG();
  }

  //////////////////////////////////////////////////////////////////////////////////
}
