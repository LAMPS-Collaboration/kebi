...
void KBHelixTrack::Remove(KBHit *hit)
{
  Double_t x = hit -> GetX();
  Double_t y = hit -> GetY();
  Double_t z = hit -> GetZ();
...
void KBHelixTrack::SetTrackID(Int_t idx)    { fTrackID = idx; }
void KBHelixTrack::SetGenfitID(Int_t idx)   { fGenfitID = idx; }
void KBHelixTrack::SetParentID(Int_t idx)   { fParentID = idx; }
...
Int_t KBHelixTrack::GetTrackID()  const  { return fTrackID; }
Int_t KBHelixTrack::GetGenfitID() const  { return fGenfitID; }
Int_t KBHelixTrack::GetParentID() const  { return fParentID; }
...
Double_t KBHelixTrack::Momentum(Double_t B) const
{
  if (fFitStatus != KBHelixTrack::kHelix && fFitStatus != KBHelixTrack::kGenfitTrack)
    return -1;
...
Double_t
KBHelixTrack::AlphaAtPosition(TVector3 p)
{
  Double_t alpha;
  TVector3 q(0,0,0);
