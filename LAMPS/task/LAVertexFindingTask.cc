...
void LAVertexFindingTask::Exec(Option_t*)
{
  ...
  auto TestVertexAtZ = [tracks, vertex](Int_t itID, TVector3 &v, bool last = false)
  {
    Double_t s = 0;
    ...
    vertex -> AddSSet(KBVSSet(itID, v.Z(), s, numUsedTracks));
  ...
  for (auto z : zArray) {
    TVector3 v(0, 0, z);
    Double_t s = TestVertexAtZ(nIterations, v);
